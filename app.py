...
class App:
    def __init__(self, root):
        ...
        self.creativity = tk.DoubleVar(value=.20)
        self.amount = tk.DoubleVar(value=1.0)
        self.clarity = tk.DoubleVar(value=0.2)
        ...
        for label, var, low, high, resolution in [('Micro-detail creativity (Maximum mode)', self.creativity, .05, .40, .01), ('Texture blend intensity', self.amount, .1, 2.5, .05), ('Micro-contrast clarity', self.clarity, 0, 2, .05)]:
            self.label(controls, label)
    ...
    def reset(self):
        ...
        self.creativity.set(.20)
        self.amount.set(1.0)
        self.clarity.set(0.2)
