...
class App:
    def __init__(self, root):
        self.root = root
        root.title('Detail Lab 8 · Fast Detail')
        ...
        tk.Label(header, text='TEXTURE STUDIO  /  08', bg=BG, fg=MUTED, font=('Segoe UI', 10)).pack(side='left', padx=18)
        tk.Label(header, text='●  LOCAL GPU', bg='#173331', fg=ACCENT, padx=16, pady=9, font=('Segoe UI', 10, 'bold')).pack(side='right')
        tk.Label(root, text='Fine texture. Familiar faces. Every pixel in view.', bg=BG, fg=MUTED, font=('Segoe UI', 12)).pack(anchor='w', padx=28)
        ...
        self.creativity = tk.DoubleVar(value=.28)
        self.amount = tk.DoubleVar(value=1.25)
        self.clarity = tk.DoubleVar(value=0.0)
        ...
        tk.Label(controls, text='Fast: one 4-step AI pass.\nMaximum: one 8-step pass at higher resolution.\n2× / 4× does not add AI passes.', wraplength=270, bg=PANEL, fg=MUTED, justify='left').pack(fill='x', pady=(0, 12))
        self.section(controls, '02', 'Texture & preservation')
        for label, var, low, high, resolution in [('AI creativity', self.creativity, .10, .30, .01), ('AI microtexture strength', self.amount, .2, 2.0, .05), ('Sharpening (optional, default off)', self.clarity, 0, 2, .05)]:
            self.label(controls, label)
    ...
    def reset(self):
        ...
        self.creativity.set(.28)
        self.amount.set(1.25)
